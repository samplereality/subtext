...
| `trashLabel` | `'Trash'` | Label on the inbox's archived-conversations section, and the header title while viewing the Trash |
...
| `debugResume()` | Play a rewind-frozen chain onward from the paused moment | [Debug mode](#debug-mode) |
...
- **The inbox screen has its own title.** The header now reads `config.inboxTitle` (default `Messages`) while the inbox is up, instead of the story's name — matching how a thread screen shows its contact and the Trash shows `config.trashLabel`.
